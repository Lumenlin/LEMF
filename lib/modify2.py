...
class BAM(nn.Module):
    def __init__(self, channel):
        ...
        self.edge_conv1 = BasicConv2d(192, channel, kernel_size=3, stride=1, padding=1)
        self.edge_conv2 = BasicConv2d(128, channel, kernel_size=3, stride=1, padding=1)

    def forward(self, x1, x3):
        size = x1.size()[2:]
        edge2 = self.edge_conv0(x1)
        edge3 = F.interpolate(x3, size, mode='bilinear', align_corners=True)
...
class CFIM(nn.Module):
    def __init__(self, channels, crop='neither', beta=None):
        self.init__ = super(CFIM, self).__init__()
...
class Network(nn.Module):
    ...
    def forward(self, x):
        ...
        # FeaFusion
        x3_fe = self.cfim1(self.up_2(x4_rf), x3_rf)  # (BS, 64, 14, 14)
        x2_fe = self.cfim2(self.up_2(x3_rf), x2_rf)  # (BS, 64, 28, 28)
        x1_fe = self.cfim3(self.up_2(x2_rf), x1_rf)  # (BS, 64, 56, 56)
