...
class CamObjDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        image = self.rgb_loader(self.images[index])  # 将图像转换成RGB模式
        gt = self.binary_loader(self.gts[index])  # 读入标注图转换为灰度图
...
def get_loader(image_root, gt_root, eg_root, batchsize, trainsize, shuffle=True, num_workers=12, pin_memory=True):
    # `num_workers=0` for more stable training
    dataset = CamObjDataset(image_root, gt_root, eg_root, trainsize)
